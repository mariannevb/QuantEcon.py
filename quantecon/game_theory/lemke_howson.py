...
def lemke_howson(g, init_pivot=0, max_iter=10**6, capping=None,
    ...
    "comprementary pivoting" (see, e.g., von Stengel [3]_ for details).
    ...
    * If `capping` is set to a positive integer, the routine is executed
      ...
      * For k = init_pivot, init_pivot + 1, ..., init_pivot + (m+n-2),
        (modulo m+n), the Lemke-Howson algorithm is executed with k as
        the initial pivot for a number `capping` of pivoting steps. If
        the algorithm converges during this loop, then the Nash
        equilibrium found is returned.

      * Otherwise, the Lemke-Howson algorithm is executed with
        init_pivot + (m+n-1) (modulo m+n) as the initial pivot until
        the total number of pivoting steps reaches `max_iter`.

      Accoding to the simulation results for *uniformly random games*,
      this heuristics outperforms the basic Lemke-Howson algorithm with
      a fixed initial pivot, where [1]_ suggests that `capping` be set
      to 10.
...
def initialize_tableaux(payoff_matrices, tableaux, bases):
    ...
    to have a column identically zero).
